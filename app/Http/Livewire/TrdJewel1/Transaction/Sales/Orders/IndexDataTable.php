...
class IndexDataTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Aksi', 'id')
                ->format(
                    fn ($value, $row, Column $column) => view('livewire.transaction.sales.orders.index-data-table-action')->withRow($row)
                ),
            Column::make('', 'id')
                ->format(
                    fn ($value, $row, Column $column) => view('livewire.transaction.sales.orders.index-data-table-action2')->withValue($row)
                )
        ];
    }
