    ...
    public function columns(): array
    {
        return [
            Column::make("Nama", "name")
                ->searchable()
                ->sortable(),
            Column::make("Kategori", "category_item.name")
                    ->searchable()
                ->sortable(),
            Column::make('Aksi','id')
                ->format(
                    fn($value, $row, Column $column) => view('livewire.masters.items.index-data-table-action')->withRow($row)
                ),
        ];
    }
