...
class Detail extends Component
{
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.price_category_id' => 'required',
        ];
        return $rules;
    }
    ...
    protected $validationAttributes = [
        ...
        'inputs.price_category_id'      => 'Kategori Harga'
    ];

    public function refreshPriceCategory()
    {
        $categoriesData = PriceCategory::orderByName()->get();
        if (!$categoriesData->isEmpty()) {
            $this->price_categories = $categoriesData->map(function ($data) {
                return [
                    'label' => $data->name,
                    'value' => $data->id,
                ];
            })->toArray();
            $this->inputs['price_category_id'] = $this->price_categories[0]['value'];
        } else {
            $this->price_categories = [];
            $this->inputs['price_category_id'] = null;
        }
    }
