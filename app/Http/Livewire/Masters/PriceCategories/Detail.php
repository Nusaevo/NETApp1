...
class Detail extends Component
{
    ...
    public function mount($action, $objectId = null)
    {
        ...
        if (($this->action === 'Edit' || $this->action === 'View') && $this->objectId) {
            $this->object = PriceCategory::withTrashed()->find($this->objectId);
            $this->status = $this->object->deleted_at ? 'Non-Active' : 'Active';
            $this->VersioNumber = $this->object->version_number;
            $this->inputs = populateArrayFromModel($this->object);
        } else {
            $this->object = new PriceCategory();
        }
    }
    ...
    {
        return view('livewire.masters.price-categories.edit');
    }
    ...
    protected function rules()
    {
        $rules = [
                ...
                'min:1',
                'max:255',
                Rule::unique('price_categories', 'name')
    ...
    public function Create()
    {
        ...
        try {
            ...
            $this->object = PriceCategory::create($objectData);
            $this->dispatchBrowserEvent('notify-swal', [
                'type' => 'success',
