    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("Total", "total_amount")
                ...
                ->sortable(),
            Column::make("Diskon", "total_discount")
                ->format(function ($value) {
                    return rupiah($value);
                })
                ->sortable(),
            ...
            Column::make('Aksi', 'id')
                ->format(
                    fn ($value, $row, Column $column) => view('livewire.transactions.sales.orders.index-data-table-action')->withRow($row)
                ),
            Column::make('', 'id')
                ->format(
                    fn ($value, $row, Column $column) => view('livewire.transactions.sales.orders.index-data-table-action2')->withValue($row)
                )
        ];
    }
