...
use App\Models\TrdTire2\Transaction\{OrderHdr, OrderDtl, DelivHdr, DelivDtl, BillingHdr, BillingDtl};
use App\Models\TrdTire2\Master\{Partner, Material, MatlUom};
...
class Detail extends BaseComponent
{
    ...
    public function saveNpwp()
    {
        ...
        try {
            ...
            if (!$partnerDetail) {
                $partnerDetail = new \App\Models\TrdTire2\Master\PartnerDetail();
   ...
   public function onMaterialChanged($key, $matl_id)
    ...
    public function goToPrintNota()
    {
        ...
        if ($this->object && $this->object->id) {
            return redirect()->route(
                'TrdTire2.Transaction.SalesOrder.PrintPdf',
                [
                    'action'   => encryptWithSessionKey('Edit'),
    ...
    public function goToPrintSuratJalan()
    {
        ...
        if ($this->object && $this->object->id) {
            return redirect()->route(
                'TrdTire2.Transaction.SalesDelivery.PrintPdf',
                [
                    'action'   => encryptWithSessionKey('Edit'),
                    'objectId' => encryptWithSessionKey($this->object->id),
                ]
