...
use App\Models\TrdTire1\Transaction\{DelivHdr, BillingHdr};
use Livewire\Attributes\On;
use App\Livewire\TrdTire1\Tax\TaxInvoicePd\IndexDataTable as TaxInvoiceTable;
...
class Index extends BaseComponent
{
    ...
    public function submitNomorFaktur()
    {
        ...
        if ($this->actionType === 'delete') {
            ...
            $delivHdrs = DelivHdr::whereIn('id', $this->selectedOrderIds)->get(['id', 'tr_code']);
            foreach ($delivHdrs as $delivHdr) {
                BillingHdr::where('tr_code', $delivHdr->tr_code)
                    ->where('tr_type', 'APB')
                    ->update(['taxinv_num' => 0]);
            }
        ...
        foreach ($delivHdrs as $delivHdr) {
            $delivId = $delivHdr->id;
            if (isset($this->taxDocNumInputs[$delivId]) && $this->taxDocNumInputs[$delivId] !== '') {
                $numRaw = trim((string) $this->taxDocNumInputs[$delivId]);
                // Validasi per baris: hanya digit dan nilai numeriknya > 0
                if ($numRaw === '' || !ctype_digit($numRaw) || intval($numRaw) <= 0) {
                    $this->dispatch('error', "Nomor faktur untuk surat jalan {$delivHdr->tr_code} tidak valid");
                    return;
                }

                // Cek unik berdasarkan string apa adanya (agar '01' berbeda dengan '1')
                $exists = BillingHdr::where('taxinv_num', $numRaw)
                    ->where('tr_code', '!=', $delivHdr->tr_code)
                    ->where('tr_type', 'APB')
                    ->where('taxinv_num', '!=', 0)
                    ->exists();
                if ($exists) {
