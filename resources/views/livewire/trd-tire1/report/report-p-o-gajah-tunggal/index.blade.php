<x-ui-page-card title="{!! $menuName !!}" status="{{ $status }}">
        ...
        <div class="card-body">
            <div class="container mb-2 mt-2">
                <div class="row align-items-end">
                    <div class="col-md-3">
                        <x-ui-dropdown-select label="Kode Program" model="selectedRewardCode" :options="$rewardOptions"
                            action="Edit" onChanged="onSrCodeChanged" />
                    </div>
                    <div class="col-md-3">
    ...
    <div class="card d-print-none"
        style="width: 100%; margin: 20px 0; background: #fff; box-shadow: 0 2px 12px rgba(0,0,0,0.08), 0 0px 1.5px rgba(0,0,0,0.03); border-radius: 10px; padding: 20px;">
        <div class="report-box" style="width: 100%; margin: auto;">
            <h3 style="text-decoration:underline; text-align:left;">
                {!! $menuName !!}
            </h3>
            <div style="text-align:left; margin-bottom:20px;">
                <strong>PROGRAM {{ $selectedRewardCode }}</strong> Periode:
                {{ $startPrintDate && $endPrintDate ? \Carbon\Carbon::parse($startPrintDate)->format('d-M-Y') . ' s/d ' . \Carbon\Carbon::parse($endPrintDate)->format('d-M-Y') : '-' }}
            </div>
            @php
                ...
                <table style="width:100%; border-collapse:collapse; font-size: 14px; min-width: 1000px;">
                    <thead>
                        @if ($isIrcBrand)
                            ...
                            <tr>
                                <th colspan="4"
                                    style="text-align:left; padding:8px 12px; border-bottom:1px solid #000; border-right:1px dashed #000; border-left:1px solid #000; border-top: 1px solid #000; background-color: #f8f9fa;">
                                    Nama / Alamat Pelanggan</th>
                                <th colspan="3"
                                    style="text-align:center; padding:8px 12px; border-right:1px dashed #000; border-top: 1px solid #000; background-color: #f8f9fa;">
                                </th>
                                <th colspan="2"
                                    style="text-align:center; padding:8px 12px; border-right:1px dashed #000; border-top: 1px solid #000; background-color: #f8f9fa;">
                                </th>
                                ...
                                <th colspan="1"
                                    style="text-align:center; padding:8px 12px; border-right:1px solid #000; border-top: 1px solid #000; background-color: #f8f9fa;">
                                </th>
                            </tr>
                            <tr class="header2">
                                <th
                                    style="text-align:left; padding:8px 12px; border-bottom:1px solid #000; border-left:1px solid #000; background-color: #f8f9fa;">
                                    Tgl SJ</th>
                                <th
                                    style="text-align:left; padding:8px 12px; border-bottom:1px solid #000; background-color: #f8f9fa;">
                                    No. Nota</th>
                                <th
                                    style="text-align:left; padding:8px 12px; border-bottom:1px solid #000; background-color: #f8f9fa;">
                                    Kode Brg.</th>
                                <th
                                    ...
                                    Nama Barang</th>
                                <th
                                    style="text-align:center; padding:8px 12px; border-bottom:1px solid #000; border-left:1px dashed #000; border-right:1px dashed #000; background-color: #f8f9fa;">
                                    Ban Luar</th>
                                <th
                                    style="text-align:center; padding:8px 12px; border-bottom:1px solid #000; border-right:1px dashed #000; background-color: #f8f9fa;">
                                    Ban Dalam</th>
                                <th
                                    style="text-align:center; padding:8px 12px; border-bottom:1px solid #000; border-right:1px dashed #000; background-color: #f8f9fa;">
                                    Total Ban</th>
                                ...
                                <th
                                    ...
                                    Total Point</th>
                                <th
                                    style="text-align:center; padding:8px 12px; border-bottom:1px solid #000; border-right:1px solid #000; background-color: #f8f9fa;">
                                    Sisa BD</th>
                            </tr>
                        @else
                    ...
                    <tbody>
                        @foreach ($results as $group)
                            ...
                            @foreach ($group['details'] as $row)
                                <tr
                                    style="background-color: {{ $loop->parent->index % 2 == 0 ? '#ffffff' : '#f8f9fa' }};">
                                    <td style="padding:8px 12px; border-left:1px solid #000;">
                                        {{ $prevNota !== $row['no_nota'] ? ($row['tgl_sj'] ? \Carbon\Carbon::parse($row['tgl_sj'])->format('d M Y') : '') : '' }}
                                    </td>
                                    <td style="padding:8px 12px;">
                                        {{ $prevNota !== $row['no_nota'] ? $row['no_nota'] : '' }}</td>
                                    ...
                                    @if ($group['is_irc'] ?? false)
                                        {{-- Format IRC --}}
                                        <td
                                            style="padding:8px 12px; text-align:center; border-left:1px dashed #000; border-right:1px dashed #000;">
                                            @if ($row['ban_luar'] ?? 0)
                                                {{ number_format($row['ban_luar'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['ban_dalam'] ?? 0)
                                                {{ number_format($row['ban_dalam'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['total_ban'] ?? 0)
                                                {{ number_format($row['total_ban'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['point_bl'] ?? 0)
                                                {{ number_format($row['point_bl'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['point_bd'] ?? 0)
                                                {{ number_format($row['point_bd'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['total_point'] ?? 0)
                                                {{ number_format($row['total_point'], 0) }}
                                            @endif
                                        </td>
                                        <td style="padding:8px 12px; border-right:1px solid #000; text-align:center;">
                                            @if ($row['sisa_bd'] ?? 0)
                                                {{ number_format($row['sisa_bd'], 0) }}
                                            @endif
                                        </td>
                                    @else
                                        {{-- Format Non-IRC --}}
                            ...
                            <tr style="font-weight: bold;">
                                <td colspan="3"
                                    style="padding:10px 12px; border-left:1px solid #000; text-align:left;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                ...
                                @if ($group['is_irc'] ?? false)
                                    {{-- Format IRC Total --}}
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000; border-left:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['ban_luar'] ?? 0)
                                            {{ number_format($group['ban_luar'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['ban_dalam'] ?? 0)
                                            {{ number_format($group['ban_dalam'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['total_ban'] ?? 0)
                                            {{ number_format($group['total_ban'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['point_bl'] ?? 0)
                                            {{ number_format($group['point_bl'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['point_bd'] ?? 0)
                                            {{ number_format($group['point_bd'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px dashed #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['total_point'] ?? 0)
                                            {{ number_format($group['total_point'], 0) }}
                                        @endif
                                    </td>
                                    <td
                                        style="padding:10px 12px; text-align:center; border-right:1px solid #000;{{ $loop->last ? ' border-bottom:1px solid #000;' : '' }}">
                                        @if ($group['sisa_bd'] ?? 0)
                                            {{ number_format($group['sisa_bd'], 0) }}
                                        @endif
                                    </td>
                                @else
                        ...
                        @if ($isIrcBrand)
                            ...
                            <tr class="header2">
                                <th
                                    ...
                                    Tgl SJ</th>
                                <th style="text-align:left; padding:4px 8px; border-bottom:1px solid #000;">No. Nota
                                </th>
                                <th style="text-align:left; padding:4px 8px; border-bottom:1px solid #000;">Kode Brg.
                                </th>
                                <th style="text-align:left; padding:4px 8px; border-bottom:1px solid #000;">Nama Barang
                                </th>
                                <th
                                    style="text-align:center; padding:4px 8px; border-bottom:1px solid #000; border-left:1px dashed #000; border-right:1px dashed #000;">
                                    Ban Luar</th>
                                <th
                            ...
                            @foreach ($group['details'] as $row)
                                <tr>
                                    ...
                                    @if ($group['is_irc'] ?? false)
                                        ...
                                        <td style="padding:4px 8px; text-align:center; border-right:1px dashed #000;">
                                            @if ($row['ban_dalam'] ?? 0)
                                                {{ number_format($row['ban_dalam'], 0) }}
                                            @endif
                                        </td>
                                        ...
                                        <td style="padding:4px 8px; border-right:1px solid #000; text-align:center;">
                                            @if ($row['sisa_bd'] ?? 0)
                                                {{ number_format($row['sisa_bd'], 0) }}
                                            @endif
                                        </td>
