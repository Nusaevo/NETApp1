<div>
    <x-ui-page-card title="{!! $menuName !!}" status="{{ $status }}">
        <div class="row mb-4">
            <div class="col-md-12">
                <x-ui-dropdown-search label="Pilih Partner" model="selectedPartnerId" optionValue="id" :query="$ddPartner['query']"
                    :optionLabel="$ddPartner['optionLabel']" :placeHolder="$ddPartner['placeHolder']" :selectedValue="$selectedPartnerId" required="false" action="Edit" enabled="true"
                    type="int" onChanged="onPartnerChanged" />
            </div>
        </div>
            <div class="card">
                <div class="card-body">
                    <x-ui-list-table id="Table" title="NPWP List (Total: {{ $npwpCount }})">
                    <x-slot name="body">
                                ...
                                <x-ui-list-body>
                                    ...
                                    <x-slot name="button">
                                        <x-ui-button clickEvent="deleteItem({{ $key }})"
                                            cssClass="btn btn-danger" iconPath="delete.svg" button-name=""
                                            :action="$actionValue" />
                                    </x-slot>
                                </x-ui-list-body>
                            </tr>
