                ...
                <tr wire:key="transaction-{{ $index }}">
                    ...
                    <x-ui-td>
                        @php
                            ...
                            switch ($row->tr_type) {
                                case 'BB': // Buy Back
                                    $trLink = '<a href="' . route('TrdRetail1.Transaction.Buyback.Detail', [
                                        'action' => encryptWithSessionKey('Edit'),
                                        'objectId' => encryptWithSessionKey($row->id)
                                    ]) . '">' . $row->tr_id . '</a>';
                                    break;
                                case 'SO': // Sales Order
                                    $trLink = '<a href="' . route('TrdRetail1.Transaction.SalesOrder.Detail', [
                                        'action' => encryptWithSessionKey('Edit'),
                                        'objectId' => encryptWithSessionKey($row->id)
                                    ]) . '">' . $row->tr_id . '</a>';
                                    break;
                    ...
                    <x-ui-td>
                        @php
                            if ($row->tr_type == 'BB') {
                                $details = \App\Models\TrdJewel1\Transaction\ReturnDtl::where('tr_id', $row->tr_id)
                                            ->where('tr_type', $row->tr_type)
                                            ->get();
                            } else {
                                $details = \App\Models\TrdJewel1\Transaction\OrderDtl::where('tr_id', $row->tr_id)
                                            ->where('tr_type', $row->tr_type)
                                            ->get();
                            }
