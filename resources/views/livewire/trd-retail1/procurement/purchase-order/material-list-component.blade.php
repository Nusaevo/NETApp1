            ...
            @foreach ($items as $key => $item)
                <tr wire:key="list{{ $key }}">
                    <x-ui-list-body>
                        ...
                        <x-slot name="rows">
                            <div class="row">
                                <x-ui-text-field model="items.{{ $key }}.matl_code"
                                    label='{{ $this->trans('code') }}' enabled="false" />
                                <x-ui-text-field model="items.{{ $key }}.barcode"
                                    label='{{ $this->trans('barcode') }}' enabled="false" />
                                <x-ui-text-field model="items.{{ $key }}.barcode"
                                    label='{{ $this->trans('barcode') }}' enabled="false" />
                            </div>
                            <div class="row">
                                <x-ui-text-field model="items.{{ $key }}.name"
                                    label='{{ $this->trans('name') }}' enabled="false" />
                                <x-ui-text-field model="items.{{ $key }}.matl_descr"
                                    label='{{ $this->trans('description') }}' enabled="false" />
                            </div>
                            <div class="row">
                                <x-ui-text-field model="items.{{ $key }}.selling_price"
                                    label='{{ $this->trans('selling_price') }}' enabled="false" />
                                <x-ui-text-field model="items.{{ $key }}.qty"
                                    label='{{ $this->trans('qty') }}' :onChanged="'changeQuantity(' . $key . ', $event.target.value)'" />
                                <x-ui-text-field model="items.{{ $key }}.price"
