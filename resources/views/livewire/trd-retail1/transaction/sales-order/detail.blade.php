                    ...
                    <div class="col-md-12">
                        <x-ui-card title="Order Items">
                            <div>
                                ...
                                <x-ui-dialog-box id="itemDialogBox" title="Search Item" width="600px"
                                    height="400px" onOpened="openItemDialogBox" onClosed="closeItemDialogBox">
                                    <x-slot name="body">
                                        <div class="row">
                                            <x-ui-text-field type="text" label="Search Code/Nama"
                                                model="searchTerm" required="true" :action="$actionValue" enabled="true"
                                                clickEvent="" buttonName="" />
                                            <!-- Table -->
                                            <x-ui-text-field-search label="Category" model="filterCategory"
                                                :options="$kategoriOptions" onChanged="" />
                                        </div>
                                        <div class="row">
                                            <x-ui-text-field-search label="Brand" model="filterBrand"
                                                :options="$brandOptions" onChanged="" />
                                            <x-ui-text-field-search label="Type" model="filterType"
                                                :options="$typeOptions" onChanged="" />
                                        </div>


                                        <x-ui-button clickEvent="searchMaterials" cssClass="btn btn-primary"
                                            button-name="Search" />
                                        <x-ui-table id="materialsTable" padding="0px" margin="0px"
                                            height="400px">
                                            <x-slot name="headers">
                                                <th class="min-w-100px">Code</th>
                                    ...
                                    <x-slot name="rows">
                                        @forelse ($return_details as $index => $item)
                                            <tr wire:key="return-{{ $index }}">
                                                ...
                                                <td style="text-align: center;">
                                                    <x-ui-text-field type="number"
                                                        model="return_details.{{ $index }}.qty" label=""
                                                        enabled="true" onChanged="" />
                                                </td>
                                                <td style="text-align: center;">{{ $item['matl_uom'] }}</td>
                                                <td style="text-align: center;">{{ rupiah($item['amt'] ?? 0) }}</td>
                                                <td style="text-align: center;">
                                                    <x-ui-button :clickEvent="'deleteReturnItem(' . $index . ')'" button-name=""
                                                        cssClass="btn-danger text-danger" iconPath="delete.svg" />
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center text-muted">Belum ada item retur
                                                </td>
                                            </tr>
                                        @endforelse
                                    </x-slot>
                    ...
                    <x-ui-footer>
                        ...
                        <div>
                            <x-ui-button :action="$actionValue"
                                clickEvent="{{ route($this->appCode . '.Transaction.SalesOrder.PrintPdf', [
                                    'action' => encryptWithSessionKey('Edit'),
                                    'objectId' => encryptWithSessionKey($object->id),
                                ]) }}"
                                cssClass="btn-primary" type="Route" loading="true" button-name="Print"
                                iconPath="print.svg" />
