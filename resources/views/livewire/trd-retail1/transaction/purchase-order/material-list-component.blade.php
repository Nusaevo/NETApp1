    ...
    <x-ui-card>
        <x-ui-table id="Table">
                    ...
                    <tr wire:key="list{{ $input_detail['id'] ?? $key }}">
                        ...
                        <td style="text-align: center;">
                            @if (!empty($input_details[$key]['image_url']))
                                <x-ui-image src="{{ $input_details[$key]['image_url'] }}" alt="Material Image" width="50px" height="50px" />
                            @else
                                <span>No Image</span>
                            @endif
                        </td>
                        ...
                        <td style="text-align: center;">
                            <x-ui-text-field model="input_details.{{ $key }}.amt_idr" label=""
                                :action="$actionValue" type="text" enabled="false" />
                        </td>
                        <td style="text-align: center;">
            ...
            <x-slot name="button">
                ...
                <x-ui-button clickEvent="openItemDialogBox" cssClass="btn btn-primary" iconPath="add.svg" button-name="Add Multiple Items" />
                ...
                <x-slot name="body">
                    <x-ui-text-field type="text" label="Search Code/Nama/Deskripsi Item"
                        model="searchTerm" required="true" :action="$actionValue" enabled="true"
                        clickEvent="searchMaterials" buttonName="Search" />
                    <!-- Table -->
                                    ...
                                    <tr wire:key="row-{{ $index }}-supplier">
                                        <td style="text-align: center;">
                                            <x-ui-option
                                                ...
                                                type="checkbox"
                                                visible="true"
                                                :options="[$material['id'] => $material['code']]"
                                                onChanged="selectMaterial({{ $material['id'] }})"
                                            />
                                        </td>
                                        <td style="text-align: center;">
                                            @if (isset($material->Attachment) && $material->Attachment->first())
                                                <img src="{{ $material->Attachment->first()->getUrl() }}" alt="Image" style="width: 50px; height: 50px;">
                                            @else
                                                <span>No Image</span>
                                            @endif
                                        </td>
