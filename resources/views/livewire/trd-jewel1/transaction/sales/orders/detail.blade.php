 ...
 <div>
    ...
    <div>
        <a href="{{ route('sales_order.index') }}" class="btn btn-link btn-color-info btn-active-color-primary me-5 mb-2"><i class="bi bi-arrow-left-circle fs-2 me-2"></i> Kembali</a>
    </div>
                                            ...
                                            <tbody >
                                                <div>
                                                    @foreach($inputs as $key => $input)
                                                        ...
                                                        <td class="border">
                                                                <input type="number" wire:model.defer='input_items.{{ $key }}.price' class="form-control @error("input_items.$key.price") is-invalid @enderror"  wire:change="$emit('sales_order_detail_change_price',{{ $key }} , $event.target.value)" placeholder="7500" required>
                                                                @error("input_items.$key.price") <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                                                        </td>
                                                        <td class="border">
                                                                <input type="number" wire:model.defer='input_items.{{ $key }}.qty' class="form-control @error("input_items.$key.qty") is-invalid @enderror"  wire:change="$emit('sales_order_detail_change_qty',{{ $key }} , $event.target.value)" placeholder="10" required>
                                                        ...
                                                        <td class="border">
                                                            <input type="number" wire:model.defer='input_items.{{ $key }}.discount' class="form-control" required>
                                                        </td>
                                                        <td>
                                                        @if (isset($this->input_items[$key]['sales_order_detail_id']))
                                                        <a  onclick="refresh({{$key}},{{$this->input_items[$key]['sales_order_detail_id']}})"  class="btn btn-primary btn-sm"><i class="bi bi-trash fs-2 me-2"></i></a>
                                                        @else
                                                        <a  onclick="refresh({{$key}},0)"  class="btn btn-primary btn-sm"><i class="bi bi-trash fs-2 me-2"></i></a>
                                                        @endif
                                                        </td>
                                                        </tr>
                                                    @endforeach
    ...
    <script type="text/javascript">
        function refresh(key,sales_order_detail_id){
            if (confirm("Apakah anda yakin akan menghapus item ini dari Nota?") == true) {
                const element = document.getElementById('div-'+key);
                element.innerHTML = '';
                livewire.emit('sales_order_detail_remove_input', key,sales_order_detail_id);
            }
        }
        ...
        $('.customersearch').on('change', function (e) {
            livewire.emit('sales_order_detail_change_customer', e.target.value);
        });
        ...
        $(document).ready(function () {
            window.addEventListener('reApplySelect2', event => {
                ...
                $('.itemsearch').on('change', function (e) {
                    ...
                    livewire.emit('sales_order_detail_change_item',e.target.id, e.target.value,index);
                });
            });
        });
