...
<div class="mb-3 responsive-field" @if(isset($span)) span="{{ $span }}" @endif @if(isset($visible) && $visible==='false' ) style="display: none;" @endif>
    ...
    <div class="text-field-container">
        <div class="responsive-input-container">
            ...
            @elseif(isset($type) && $type === 'barcode')
            <input wire:model="{{ $model }}" id="{{ $id }}" type="text" class="responsive-input form-control @error($model) is-invalid @enderror" @if(isset($action) && $action=='View' || (!empty($enabled) && $enabled==='false' )) disabled @endif @if(isset($required) && $required==='true' ) required @endif placeholder="{{ isset($placeHolder) ? $placeHolder : '' }}" autocomplete="off" wire:change="{{ isset($onChanged) ? $onChanged : '' }}" autocomplete="off"/>
            ...
            <input wire:model.defer="{{ $model }}" id="{{ $id }}" type="text" class="inputDates responsive-input form-control @error($model) is-invalid @enderror" @if(isset($action) && $action=='View' || (!empty($enabled) && $enabled==='false' )) disabled @endif @if(isset($required) && $required==='true' ) required @endif wire:change="{{ isset($onChanged) ? $onChanged : '' }}" readonly="readonly" />
            ...
            <input wire:model.defer="{{ $model }}" id="{{ $id }}" type="text" class="inputNumbers responsive-input form-control @error($model) is-invalid @enderror" @if(isset($action) && $action=='View' || (!empty($enabled) && $enabled==='false' )) disabled @endif @if(isset($required) && $required==='true' ) required @endif wire:change="{{ isset($onChanged) ? $onChanged : '' }}" autocomplete="off">
