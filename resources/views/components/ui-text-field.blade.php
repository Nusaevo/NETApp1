    ...
    <div class="responsive-input-container">
        ...
        @elseif(isset($type) && $type === 'barcode')
            <input wire:model="{{ $model }}" id="{{ $model }}" type="text"
                   class="responsive-input form-control @error($model) is-invalid @enderror"
                   @if(isset($action) && $action == 'View' || (!empty($enabled) && $enabled === 'false')) disabled @endif
                   @if(isset($required) && $required === 'true') required @endif
                   placeholder="{{ isset($placeHolder) ? $placeHolder : '' }}"
                   autocomplete="off" wire:change="{{ isset($onChanged) ? $onChanged : '' }}" />
